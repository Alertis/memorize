...
async function buildBundle(args, ctx, output = _bundle().default) {
  ...
  if (config.resolver.platforms.indexOf(args.platform) === -1) {
    _logger.default.error(`Invalid platform ${args.platform ? `"${_chalk().default.bold(args.platform)}" ` : ''}selected.`);

    _logger.default.info(`Available platforms are: ${config.resolver.platforms.map(x => `"${_chalk().default.bold(x)}"`).join(', ')}. If you are trying to bundle for an out-of-tree platform, it may not be installed.`);
